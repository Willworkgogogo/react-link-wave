...
var DEFAULT_COLOR = '#0000EE';
var STANDARD_COLOR_HEX_LENGTH = [4, 7];
...
function LinkWave(_ref) {
  ...
  var formatColor = function formatColor(colorHex) {
    if (typeof colorHex === 'string' && colorHex) {
      if (colorHex.indexOf('#') === 0 && STANDARD_COLOR_HEX_LENGTH.includes(colorHex.length)) {
        return colorHex;
      } else {
        return "#".concat(colorHex);
      }
    } else {
      console.error('the color you set is non-compliant, it should like #0000EE');
    }
  ...
  var dealColor = function dealColor(style) {
    var cacheStyle = Object.assign({}, style);
